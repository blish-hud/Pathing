...
namespace BhModule.Community.Pathing.State {
    public class ApiStates : ManagedState {

        private static readonly Logger Logger = Logger.GetLogger<ApiStates>();

        private const double INTERVAL_CHECKAPI = 300010; // 5 minutes + 10ms

        private double _lastApiCheck = INTERVAL_CHECKAPI;

        private HashSet<string> _completedRaids;

        public ApiStates(IRootPackState rootPackState) : base(rootPackState) { }
